...
object Control {
  ...
  private[effekt] final def shift[A, E, FX](p: Scope[E, FX])(f: CPS[A, Control[E, FX]]): A / p.type =
  ...
  private[effekt] final def resetWith[E, FX](p: Scope[E, FX])(f: E / (p.type & FX)): E / FX =
    new Control[E, FX] {
      def apply[R2](k: MetaCont[E, R2]): Result[R2] = {
  ...
  private[effekt] final def delimitState[R, E](s: State)(f: R / (s.effect & E)): R / E =
    new Control[R, E] {
