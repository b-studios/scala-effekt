...
//   def me: NominalPhrase using Speaker = s ?=> s.speaker()
// }
...
//   def implicate(s: Sentence): Unit using Implicature = i ?=> i.implicate(s)
// }
...
//   def quote(f: Sentence using Speaker): Speaker => Control[Sentence] = s => f(using s)
